Public Class frmCreateApiControllerCJ
    ...
    Private Sub btnGenerate_Click(sender As Object, e As EventArgs) Handles btnGenerate.Click
        ...
        l1.Add(<![CDATA[   string _connectionString; ]]>.Value)
        l1.Add(<![CDATA[   public PositionController(IConfiguration configuration) { ]]>.Value)
        l1.Add(<![CDATA[    _connectionString = configuration.GetConnectionString("connectionString").ToString(); ]]>.Value)
        l1.Add(<![CDATA[   } ]]>.Value)
        l1.Add("")
        ...
        For i = 0 To txtModels.Lines.Count - 1
            ...
            If i = 0 Then
                ...
                l1.Add(<![CDATA[ [HttpGet("index")] ]]>.Value.Replace("index", v.ToLower))
                l1.Add(<![CDATA[ public string Index() { return "Nothing to see here ... "; } ]]>.Value)
                l1.Add("")
            End If

            If chkList.Checked Then
                Dim ep = "model/action".Replace("model/action", $"{r}/{txtControllerName.Text.ToLower}/getall{v}".ToLower)
                l2.Add($" {ep}".ToLower & " - GET method to return a list ")
                l1.Add(<![CDATA[ // GET: *endpoint* ]]>.Value.Replace("*endpoint*", ep).ToLower)
                l1.Add(<![CDATA[ [HttpGet("*endpoint*")] ]]>.Value.Replace("*endpoint*", v.ToLower.Replace(r.ToLower, "")))
                l1.Add(<![CDATA[ public IActionResult GetProducts() { ]]>.Value.Replace("GetProducts", $"GetAll{v}"))
                l1.Add(<![CDATA[    List<SectionModel> list = new SectionModelDataAccess(_connectionString).List();]]>.Value.Replace("SectionModel", $"{v}Model"))
                l1.Add(<![CDATA[    if (list != null && list.Count > 0) { return Ok(list); } ]]>.Value)
                l1.Add(<![CDATA[    return BadRequest(); ]]>.Value)
                l1.Add(<![CDATA[ } ]]>.Value)
            End If
